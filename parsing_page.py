...
def get_info_product(link):
    ...
    # get the all info from the table with pandas
    table = pd.read_html(link)
    table = str(table[0]).split('  ')
    new_table = [elem for elem in table if len(str(elem)) > 0]
    final_result["universal_product_code"] = new_table[3][:-2]
    final_result["price_excluding_tax"] = new_table[7][:-2]
    final_result["price_including_tax"] = new_table[9][:-2]
    final_result["number_available"] = new_table[13][:-2]

    # get the all info from the table with soup
...
""" The comment under this message is here if you 
want to get only one data from a specific book """
# data = get_info_product("link to place here")
# # # export with pandas
# for key, value in data.items():
#     data[key] = [value]
# dataframe= pd.DataFrame(data)
# dataframe.to_excel('only_one_books.xlsx')